...
<head>
  ...
  $id = $_GET['id'];
  $obj = new Crud_Pessoa();
  //Select dos usuários da tabela
  $linha =$obj->empresa_select_table();
  //Select último usuário
  $ultima_empresa = $obj->empresa_select_ultimo();
      ...
      <div class="card-body">
        ...
        <div class="modal fade" id="modalalterar" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="modalalterar">Alterar Empresa</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
              <?php
              require('alterar_empresa.php');
              ?>
            </div>
          </div>
        </div>
       ...
       <div class="table-responsive">
        ...
        <tbody>
         ...
         while ($i < count($linha)) {
          ...
          <td><?php echo  $linha[$i]['razaosocial'] ?></td>
          <td><?php echo $linha[$i]['nomefantasia'] ?></td>
          <td><?php echo $linha[$i]['telefone'] ?></td>
          <td><?php echo  $linha[$i]['cnpj'] ?></td>
          <td><?php echo  $linha[$i]['email'] ?></td>
          <td>
            <a data-toggle="tooltip" data-placement="top" onclick="return confirm('Tem certeza que deseja excluir?');" class="btn btn-danger btn-lg" title="Excluir" href="classes/empresa_delete.php?id=<?php echo $linha[$i]['id']?>"> 
              <i data-feather="trash-2"></i>
            </a>
            <button type="button" class="btn btn-info btn-lg" data-toggle="modal" data-target="#modalalterar" 
            data-id="<?php echo $linha[$i]['id']; ?>" 
            data-razaosocial="<?php echo $linha[$i]['razaosocial']; ?>" 
            data-nomefantasia="<?php echo $linha[$i]['nomefantasia']; ?>" 
            data-telefone="<?php echo $linha[$i]['telefone']; ?>" 
            data-cnpj="<?php echo $linha[$i]['cnpj']; ?>"
